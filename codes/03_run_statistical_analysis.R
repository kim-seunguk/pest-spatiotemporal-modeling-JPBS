...
# main pipeline (02) and the raw data from the phenology script (01).
#
...
if (length(uncertainty_files) == 0 || length(prediction_files) == 0) {
  stop(
    "Prediction/Uncertainty rasters not found. Please run '02_run_xgboost_pipeline.R' first."
  )
}
...
if (length(shap_files) == 0) {
  stop(
    "SHAP importance files (.rds) not found. Please run '02_run_xgboost_pipeline.R' first."
  )
}
